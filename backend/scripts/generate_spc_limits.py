...
def generate_spc_limits():
            ...
            for product_type in product_types:
                for chart_name in spc_chart_names:
                    ...
                    should_create_limits = (
                        schedule["current_limits"] is None or  # First time
                        current_date >= schedule["next_change"]  # Time for change
                    )
                    ...
                    if should_create_limits:
                        ...
                        days_until_next = np.clip(days_until_next, 90, 365)  # 3 months to 1 year
                        schedule["next_change"] = current_date + timedelta(days=days_until_next)
    ...
    for data in sample_data:
        print(f"  {data.process_type}/{data.product_type}/{data.spc_chart_name} - "
    ...
    for chart_name in spc_chart_names:
        chart_count = db.query(SPCLimits).filter(SPCLimits.spc_chart_name == chart_name).count()
