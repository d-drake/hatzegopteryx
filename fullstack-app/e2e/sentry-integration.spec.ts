...
test.describe('Sentry Integration', () => {
  ...
  test('should capture errors and send to Sentry', async ({ page }) => {
    ...
    if (sentryRequests.length > 0) {
      console.log('Error should be visible in Sentry project: hatzegopteryx-frontend');
  ...
  test('should handle API errors gracefully', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Hatzegopteryx');
  });
  ...
  test('should be properly configured with DSN', async ({ page }) => {
    ...
    if (config.dsnProjectId) {
      ...
      console.log('You can verify errors in Sentry at: https://pdev-zx.sentry.io/projects/hatzegopteryx-frontend/');
    }
  });
});