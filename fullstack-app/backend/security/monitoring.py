...
class SecurityMonitor:
    ...
    def check_failed_login_attempts(self, user_id: Optional[int] = None, ip_address: Optional[str] = None) -> Dict:
        """Check for excessive failed login attempts."""
        window_start = datetime.now(timezone.utc) - timedelta(minutes=self.alert_thresholds["failed_logins"]["window_minutes"])
    ...
    def check_registration_spam(self, ip_address: str) -> Dict:
        ...
        window_start = datetime.now(timezone.utc) - timedelta(minutes=self.alert_thresholds["registration_attempts"]["window_minutes"])
        ...
        return {
            ...
            "ip_address": ip_address,
            "window_minutes": self.alert_thresholds["registration_attempts"]["window_minutes"]
        }
    
    def check_unauthorized_access(self, user_id: int) -> Dict:
        """Check for repeated unauthorized access attempts."""
        window_start = datetime.now(timezone.utc) - timedelta(minutes=self.alert_thresholds["unauthorized_access"]["window_minutes"])
        ...
        return {
            ...
            "triggered": attempts >= threshold,
            "user_id": user_id,
            "window_minutes": self.alert_thresholds["unauthorized_access"]["window_minutes"]
        }
    ...
    def send_alert(self, alert_data: Dict, recipients: List[str]):
        ...
        try:
            msg = MIMEMultipart()
            msg["From"] = os.getenv("SMTP_FROM", "security@hatzegopteryx.com")
            msg["To"] = ", ".join(recipients)
            msg["Subject"] = f"Security Alert: {alert_data.get('alert_type', 'Unknown')}"
    ...
    def _format_alert_email(self, alert_data: Dict) -> str:
        ...
        html += """
            ...
            <p style="margin-top: 20px; color: #666;">
                This is an automated security alert from Hatzegopteryx Security System.
            </p>
        </body>
        </html>
        """
...
class SecurityScanner:
    ...
    def check_weak_passwords(self) -> List[Dict]:
        ...
        old_accounts = self.db.query(User).filter(
        ...
        return [{
            ...
            "email": user.email,
            "days_since_created": (datetime.now(timezone.utc) - user.created_at).days,
    ...
    def check_inactive_users(self) -> List[Dict]:
        ...
        inactive_users = self.db.query(User).outerjoin(
            last_activities,
            User.id == last_activities.c.user_id
        ).filter(
            and_(
                User.is_active == True,
                func.coalesce(last_activities.c.last_activity, User.created_at) < cutoff_date
            )
        ).all()
    ...
    def check_privilege_escalation(self) -> List[Dict]:
        ...
        privilege_changes = self.db.query(AuditLog).filter(
            and_(
