...
@router.get("/alerts/failed-logins", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/alerts/registration-spam", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/alerts/unauthorized-access", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/alerts/suspicious-patterns", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/scan/weak-passwords", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/scan/inactive-users", dependencies=[Depends(get_current_active_superuser)])
...
@router.get("/scan/privilege-escalation", dependencies=[Depends(get_current_active_superuser)])
...
def security_dashboard(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "recommendations": {
            "immediate": [
                f"Review {len(privilege_changes)} privilege escalation alerts" if privilege_changes else None,
                f"Investigate {len(suspicious_patterns)} suspicious activity patterns" if suspicious_patterns else None
            ],
            "scheduled": [
                f"Review password policy for {len(weak_passwords)} users with old accounts" if weak_passwords else None,
                f"Disable {len(inactive_users)} inactive accounts" if inactive_users else None
            ]
        },
        "health_status": "critical" if privilege_changes or len(suspicious_patterns) > 5 else "warning" if suspicious_patterns or weak_passwords else "good"
    }