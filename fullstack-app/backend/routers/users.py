...
def get_pending_registrations(db: Session = Depends(get_db)):
    ...
    registrations = (
        ...
        .filter(
            RegistrationRequest.approved.is_(None),
...
@router.get(
    "/{user_id}", response_model=UserResponse, dependencies=[Depends(require_superuser)]
)
...
@router.post(
    "/", response_model=UserResponse, dependencies=[Depends(require_superuser)]
)
def create_user(
...
def update_user(
    ...
    create_audit_log(
        db, current_user.id, "update_user", f"user:{user.id}", True, details=update_data
    )
...
def approve_registration(
    ...
    registration = (
        db.query(RegistrationRequest)
        .filter(RegistrationRequest.id == registration_id)
        .first()
    )
    ...
    background_tasks.add_task(
        send_registration_approved_email, new_user.email, new_user.username, app_url
    )
...
@router.post(
    "/reject-registration/{registration_id}", dependencies=[Depends(require_superuser)]
)
def reject_registration(
    ...
    background_tasks.add_task(
        send_registration_rejected_email, registration.email, registration.username
    )
