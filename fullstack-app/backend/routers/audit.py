...
def get_audit_logs(
    ...
    return [
        AuditLogResponse(
            ...
            user_id=log.user_id,
            user=(
                {"username": log.user.username, "email": log.user.email}
                if log.user
                else None
            ),
            action=log.action,
...
@router.get(
    "/actions", response_model=List[str], dependencies=[Depends(require_superuser)]
)
...
def get_audit_stats(days: int = Query(default=7, le=30), db: Session = Depends(get_db)):
    """
    ...
    failed_attempts = (
        db.query(AuditLog)
        .filter(and_(AuditLog.created_at >= start_date, AuditLog.success.is_(False)))
        .count()
    )
    ...
    active_users = (
        db.query(
            User.username, User.email, db.func.count(AuditLog.id).label("action_count")
        )
        .join(AuditLog)
    ...
    return {
        ...
        "most_active_users": [
            {"username": u[0], "email": u[1], "actions": u[2]} for u in active_users
        ],
    }
