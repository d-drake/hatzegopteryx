...
from schemas import (
    LoginRequest, Token, TokenRefresh, UserResponse, 
    RegistrationRequest, RegistrationResponse, UserCreate
)
from auth import (
    authenticate_user, create_access_token, create_refresh_token,
    get_current_user, get_password_hash, save_refresh_token,
    invalidate_refresh_token, invalidate_all_user_tokens,
    verify_token, create_audit_log, blacklist_token, REFRESH_TOKEN_EXPIRE_DAYS,
    security
)
...
async def register(
    ...
    existing_user = db.query(User).filter(
        (User.email == registration.email) | (User.username == registration.username)
    ).first()
    
    if existing_user:
        create_audit_log(
            db, None, "registration_attempt", registration.email, False,
            request, {"reason": "email_or_username_exists"}
        )
        raise HTTPException(
    ...
    ).filter(RegistrationRequestModel.approved == None).first()
    ...
    create_audit_log(
        db, None, "registration_request", registration.email, True,
        request, {"username": registration.username}
    )
...
async def login(
    ...
    if not user:
        create_audit_log(
            db, None, "login_attempt", login_data.email, False,
            request, {"reason": "invalid_credentials"}
        )
        raise HTTPException(
    ...
    create_audit_log(
        db, user.id, "login", None, True, request
    )
...
async def logout(
    ...
    create_audit_log(
        db, current_user.id, "logout", None, True, request
    )
...
async def refresh_token(
    ...
    try:
        ...
        create_audit_log(
            db, user.id, "token_refresh", None, True, request
        )
        ...
        create_audit_log(
            db, None, "token_refresh", None, False,
            request, {"error": str(e)}
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
