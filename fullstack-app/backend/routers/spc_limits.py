...
def get_spc_limits(
    ...
    if effective_date:
        filters.append(
            models.SPCLimits.effective_date
            <= datetime.combine(effective_date, datetime.max.time())
        )
...
def get_current_spc_limits(
    ...
    if spc_monitor_name:
        subquery = subquery.filter(
            models.SPCLimits.spc_monitor_name == spc_monitor_name
        )
    if spc_chart_name:
...
def create_spc_limit(spc_limit: schemas.SPCLimitsCreate, db: Session = Depends(get_db)):
    ...
    if spc_limit.spc_monitor_name == "SPC_CD_L1":
        if spc_limit.spc_chart_name == "cd_att":
            if spc_limit.cl is not None and not (-5 <= spc_limit.cl <= 5):
                raise HTTPException(
                    status_code=400, detail="CL must be between -5 and 5 for cd_att"
                )
            if spc_limit.lcl is not None and not (-60 <= spc_limit.lcl <= -30):
                raise HTTPException(
                    status_code=400, detail="LCL must be between -60 and -30 for cd_att"
                )
            if spc_limit.ucl is not None and not (30 <= spc_limit.ucl <= 60):
                raise HTTPException(
                    status_code=400, detail="UCL must be between 30 and 60 for cd_att"
                )
        elif spc_limit.spc_chart_name == "cd_x_y":
            if spc_limit.cl is not None and not (-2 <= spc_limit.cl <= 2):
                raise HTTPException(
                    status_code=400, detail="CL must be between -2 and 2 for cd_x_y"
                )
            if spc_limit.lcl is not None and not (-10 <= spc_limit.lcl <= -6):
                raise HTTPException(
                    status_code=400, detail="LCL must be between -10 and -6 for cd_x_y"
                )
            if spc_limit.ucl is not None and not (6 <= spc_limit.ucl <= 10):
                raise HTTPException(
                    status_code=400, detail="UCL must be between 6 and 10 for cd_x_y"
                )
        elif spc_limit.spc_chart_name == "cd_6sig":
            if spc_limit.cl is not None and not (10 <= spc_limit.cl <= 30):
                raise HTTPException(
                    status_code=400, detail="CL must be between 10 and 30 for cd_6sig"
                )
            if spc_limit.lcl is not None:
                raise HTTPException(
                    status_code=400, detail="LCL must be null for cd_6sig"
                )
            if spc_limit.ucl is not None and not (55 <= spc_limit.ucl <= 75):
                raise HTTPException(
                    status_code=400, detail="UCL must be between 55 and 75 for cd_6sig"
                )
