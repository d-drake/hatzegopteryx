...
def cleanup_cd_data(dry_run=True, force=False):
    ...
    try:
        ...
        print(f"3. Process_Type == '1100' AND Product_Type IN ('VLQR1', 'XLY1', 'XLY2'): {deletion3_count:,} records")
        ...
        print(f"Will remain: {remaining_count:,} records ({100 - deletion_percentage:.1f}%)")
        
        # Check SPC limits that would be affected
        print("\n=== SPC Limits Impact ===")
        
        # SPC limits for Process_Type 900
        spc_limits_900 = session.query(func.count(SPCLimits.id)).filter(
            SPCLimits.process_type == '900'
        ).scalar()
        
        # SPC limits for specific process/product combinations
        spc_limits_1000_xly1 = session.query(func.count(SPCLimits.id)).filter(
            and_(
        ...
        spc_limits_1100_affected = session.query(func.count(SPCLimits.id)).filter(
            and_(
        ...
        total_spc_affected = spc_limits_900 + spc_limits_1000_xly1 + spc_limits_1100_affected
        ...
        if not dry_run:
            ...
            print(f"✓ Deleted {deleted2:,} records where Process_Type == '1000' AND Product_Type == 'XLY1'")
            
            deleted3 = deletion3_query.delete(synchronize_session=False)
            print(f"✓ Deleted {deleted3:,} records where Process_Type == '1100' AND Product_Type IN ('VLQR1', 'XLY1', 'XLY2')")
            
            # Delete corresponding SPC limits
            spc_deleted1 = session.query(SPCLimits).filter(
                SPCLimits.process_type == '900'
            ).delete(synchronize_session=False)
            
            spc_deleted2 = session.query(SPCLimits).filter(
                and_(
                    SPCLimits.process_type == '1000',
                    SPCLimits.product_type == 'XLY1'
                )
            ...
            spc_deleted3 = session.query(SPCLimits).filter(
                and_(
                    SPCLimits.process_type == '1100',
                    SPCLimits.product_type.in_(['VLQR1', 'XLY1', 'XLY2'])
                )
            ...
            if final_count == remaining_count:
                ...
                print(f"⚠️  WARNING: Record count mismatch! Expected {remaining_count:,}, got {final_count:,}")
