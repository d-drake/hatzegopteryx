...
def check_ssh_tunnel():
    ...
    try:
        result = subprocess.run(["lsof", "-ti:5432"], capture_output=True, text=True)
...
def main():
    ...
    if not check_ssh_tunnel():
        ...
        log_message(
            "Run: cd /home/dwdra/workspace/hatzegopteryx/infrastructure/scripts",
            "ERROR",
        )
    ...
    if not prod_password:
        log_message(
            "Please enter the production database password (from terraform.tfvars):"
        )
        try:
            ...
            log_message(
                "You can also set PROD_DB_PASSWORD environment variable", "ERROR"
            )
            return 1
    ...
    for table_name in TABLES_TO_MIGRATE:
        ...
        if dev_count == prod_count:
            ...
            log_message(
                f"✗ {table_name}: MISMATCH dev={dev_count:,} prod={prod_count:,}",
                "ERROR",
            )
            all_verified = False
