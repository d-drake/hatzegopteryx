...
import { fetchSPCRegL1Data, SPCRegL1Item } from '@/services/spcRegL1Service';
import { FilterState } from '@/components/spc-dashboard/FilterControls';

interface SPCRegL1ContextValue {
  data: SPCRegL1Item[];
  allEntityData: SPCRegL1Item[]; // Data without entity filter
  isLoading: boolean;
  error: string | null;
  filters: FilterState;
...
const SPCRegL1Context = createContext<SPCRegL1ContextValue | null>(null);

interface SPCRegL1ProviderProps {
...
const dataCache = new Map<string, { data: SPCRegL1Item[], timestamp: number }>();
const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes

export function SPCRegL1Provider({ 
  children, 
  processType, 
  productType, 
  spcMonitorName,
  processProduct
}: SPCRegL1ProviderProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  const [data, setData] = useState<SPCRegL1Item[]>([]);
  const [allEntityData, setAllEntityData] = useState<SPCRegL1Item[]>([]);
  ...
  const getDefaultFilters = (): FilterState => {
    ...
    const filters = {
      entity: 'FAKE_TOOL1', // Default to first entity
  ...
  const loadFilteredData = useCallback(async () => {
    ...
    try {
      ...
      const allDataResponse = await fetchSPCRegL1Data(baseParams);
  ...
  useEffect(() => {
    ...
    const hasUrlFilters = urlFilters.entity !== 'FAKE_TOOL1' || urlFilters.startDate !== '' || urlFilters.endDate !== '';
  ...
  const value: SPCRegL1ContextValue = useMemo(() => ({
    data,
    allEntityData,
    isLoading,
    error,
  ...
  return (
    <SPCRegL1Context.Provider value={value}>
      {children}
    </SPCRegL1Context.Provider>
  );
}

export function useSPCRegL1() {
  const context = useContext(SPCRegL1Context);
  if (!context) {
    throw new Error('useSPCRegL1 must be used within SPCRegL1Provider');
  }
  return context;
}