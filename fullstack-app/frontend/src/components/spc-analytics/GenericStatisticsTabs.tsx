...
export default function GenericStatisticsTabs<T extends SPCDataItem>({ 
  ...
  const [activeTab, setActiveTab] = useState<string>(defaultMetric || metrics[0]?.key || '');
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      {limitsLoading ? (
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
          <h3 className="text-sm font-medium text-gray-700 mb-2">Current Control Limits</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div>
              <span className="text-gray-500">UCL: </span>
              <span className="font-medium text-gray-900">
                {activeLimits.ucl !== null && activeLimits.ucl !== undefined ? 
                  formatWithUnit(activeLimits.ucl) : 'Not set'}
              </span>
            </div>
            <div>
              ...
              <span className="font-medium text-gray-900">
                {activeLimits.cl !== null && activeLimits.cl !== undefined ? 
                  formatWithUnit(activeLimits.cl) : 'Not set'}
              </span>
            </div>
            <div>
              ...
              <span className="font-medium text-gray-900">
                {activeLimits.lcl !== null && activeLimits.lcl !== undefined ? 
                  formatWithUnit(activeLimits.lcl) : 'Not set'}
              </span>
            </div>
            <div>
        ...
        <div className="mb-6 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
          <p className="text-sm text-yellow-800">No control limits defined for {activeMetricConfig?.label || activeTab}</p>
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {statistics.map((stat) => (
              <tr key={stat.entity} className={stat.entity === 'All' ? 'bg-gray-50' : ''}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {stat.entity}
                  {stat.withinLimits !== undefined && (
                    <span className={`ml-2 text-xs ${stat.withinLimits ? 'text-green-600' : 'text-red-600'}`}>
                      {stat.withinLimits ? '✓' : '✗'}
                    </span>
                  )}
                </td>
                <td className={`px-6 py-4 whitespace-nowrap text-sm ${
                  isOutsideLimits(stat.mean) ? 'text-red-600 font-semibold' : 'text-gray-900'
                }`}>
