...
interface BoxPlotRendererProps {
  ...
  onBoxHover: (
    data: Omit<Extract<HoverData, { type: "box" }>, "type">,
    event: MouseEvent,
  ) => void;
  /** Set hover data for individual data points */
  onPointHover: (
    data: Omit<Extract<HoverData, { type: "point" }>, "type">,
    event: MouseEvent,
  ) => void;
  /** Set hover data for mean markers */
  onMeanHover: (
    data: Omit<Extract<HoverData, { type: "mean" }>, "type">,
    event: MouseEvent,
  ) => void;
    ...
    boxPlotData.forEach((boxData) => {
      ...
      data
        ...
        .forEach((d, i) => {
          ...
          const isOutlier =
            value < boxData.lowerWhisker || value > boxData.upperWhisker;
          const jitter =
            (seedRandom(i + boxData.entity.charCodeAt(0)) - 0.5) * jitterWidth;
