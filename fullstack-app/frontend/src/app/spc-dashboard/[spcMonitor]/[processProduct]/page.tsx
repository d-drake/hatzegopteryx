...
function SPCCdL1DashboardInner() {
          ...
          {isGuest && (
            <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-yellow-800">
                <strong>Notice:</strong> Access to the SPC Dashboard is limited
                to the past 30 days only for guests. Please{" "}
...
function SPCRegL1DashboardInner() {
  const params = useParams();
  const { user } = useAuth();
  const {
    data,
          ...
          {!loading && !error && data.length > 0 && (
            <div className="mt-8">
              ...
              <div className="space-y-8">
                ...
                {/* Centrality Rotation vs Date */}
                <SPCChartWithSharedData
                  title="Centrality Rotation vs Date"
                  data={data}
                  yField="centrality_rotation"
                  processType={processType}
                  productType={productType}
                  spcMonitor={spcMonitor}
                  syncViews={syncViews}
...
function SPCDashboardContent() {
  ...
  // Route to the appropriate dashboard based on SPC monitor type
  if (spcMonitor.includes("REG_L1")) {
    return (
      <SPCRegL1Provider
        processType={processType}
        productType={productType}
