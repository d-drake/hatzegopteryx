...
const METRICS_DIR = path.join(
  __dirname,
  "../src/lib/spc-dashboard/config/metrics",
);
...
function convertYamlToTs(yamlFilePath) {
  try {
    ...
    const configName = baseName.replace(/-([a-z])/g, (match, letter) =>
      letter.toUpperCase(),
    );
...
export const ${configName}Config: SPCMonitorConfig = ${JSON.stringify(config, null, 2)};`;
    ...
    console.log(
      `✅ Converted ${path.basename(yamlFilePath)} to ${path.basename(tsFilePath)}`,
    );
  } catch (error) {
...
function main() {
  ...
  console.log(
    "   The TypeScript files are generated and should not be edited directly.",
  );
}
